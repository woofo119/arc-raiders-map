...
const SkillTreePage = () => {
    ...
    const [activeTab, setActiveTab] = useState('conditioning'); // 'conditioning', 'mobility', 'survival'
    ...
    // 카테고리별로 라인 분류 (x 좌표 기준)
    const getLineCategory = (x) => {
        if (x < 35) return 'conditioning';
        if (x < 60) return 'mobility';
        return 'survival';
    };
    ...
    return (
        <div className="flex-1 relative h-full bg-[#0a0a0a] overflow-hidden flex flex-col text-white">
            {/* Top Navigation Bar */}
            <div className="h-14 border-b border-gray-800 flex items-center justify-between px-4 md:px-6 bg-gray-900/50 backdrop-blur-sm z-30">
            ...
            {/* Mobile Tab Bar */}
            <div className="flex md:hidden border-b border-gray-800 bg-gray-900/80 backdrop-blur-sm z-20">
                {[
                    { id: 'conditioning', label: '단련', color: 'text-green-500', border: 'border-green-500' },
                    { id: 'mobility', label: '이동성', color: 'text-yellow-500', border: 'border-yellow-500' },
                    { id: 'survival', label: '생존', color: 'text-red-500', border: 'border-red-500' }
                ].map(tab => (
                    <button
                        key={tab.id}
                        onClick={() => setActiveTab(tab.id)}
                        className={`flex-1 py-3 text-sm font-bold transition-all relative ${activeTab === tab.id ? `${tab.color}` : 'text-gray-500'
                            }`}
                    >
                        {tab.label} ({points[tab.id]})
                        {activeTab === tab.id && (
                            <div className={`absolute bottom-0 left-0 right-0 h-0.5 ${tab.border.replace('border', 'bg')}`} />
                        )}
                    </button>
                ))}
            </div>

            {/* Desktop Header / Points Display (Floating) */}
            <div className="hidden md:block absolute top-20 left-0 right-0 h-20 z-20 pointer-events-none">
            ...
            <div className="flex-1 relative w-full h-full overflow-hidden bg-[#0a0a0a]">
                <div className="relative w-full h-full md:min-w-[1000px] md:min-h-[800px] md:overflow-auto custom-scrollbar">
                    ...
                    <svg className="absolute inset-0 w-full h-full pointer-events-none">
                        {lines.map((line, i) => {
                            // 모바일에서는 현재 탭에 해당하는 라인만 표시
                            // 데스크톱에서는 모든 라인 표시
                            const category = getLineCategory(line.x1);

                            // 모바일 탭 필터링
                            // 모바일 뷰포트(md 미만)에서만 필터링 적용하려면 CSS 클래스로 제어하거나
                            // JS로 window width를 체크해야 함. 
                            // 여기서는 간단히 CSS 클래스로 제어하기 위해 모든 라인을 렌더링하되
                            // 클래스로 숨김 처리를 함.

                            // 좌표 변환: 모바일에서는 각 트리를 중앙으로 이동시켜야 함.
                            // 단련(25%) -> 50%, 이동성(50%) -> 50%, 생존(75%) -> 50%
                            // x 좌표 이동량 계산
                            let mobileX1 = line.x1;
                            let mobileX2 = line.x2;

                            if (category === 'conditioning') {
                                mobileX1 += 25; mobileX2 += 25;
                            } else if (category === 'survival') {
                                mobileX1 -= 25; mobileX2 -= 25;
                            }
                            // 이동성은 이미 50% 근처이므로 그대로
                            ...
                            return (
                                <g key={i}>
                                    ...
                                    <line
                                        ...
                                        stroke="#333"
                                        strokeWidth="2"
                                        className={`opacity-60 md:hidden ${activeTab === category ? 'block' : 'hidden'}`}
                                    />
                                </g>
                            );
                        })}
                    ...
                    {allSkills.map(skill => {
                        ...
                        // 모바일 위치 보정
                        let mobileStyle = { left: `${skill.x}%`, top: `${skill.y}%` };
                        if (skill.category === 'conditioning') {
                            mobileStyle.left = `${skill.x + 25}%`;
                        } else if (skill.category === 'survival') {
                            mobileStyle.left = `${skill.x - 25}%`;
                        }

                        // 렌더링 여부 결정 (모바일)
                        const isVisibleMobile = activeTab === skill.category;
                        ...
                        return (
                            <React.Fragment key={skill.id}>
                                ...
                                {/* Mobile Node (Shifted & Filtered) */}
                                {isVisibleMobile && (
                                    <div
                                        className="absolute transform -translate-x-1/2 -translate-y-1/2 flex flex-col items-center md:hidden"
                                        style={mobileStyle}
                                    >
                                        <SkillNode
                                            skill={skill}
                                            ...
                                            onAdd={(id) => {
                                                ...
                                                // 모바일에서는 클릭 시 레벨업 하지 않고 정보창만 띄움 (실수 방지)
                                            }}
                                            onRemove={(id) => {
                                                setSelectedSkill(skill);
                                            }}
                ...
                <div className="fixed bottom-0 left-0 right-0 bg-gray-900 border-t border-gray-700 p-4 z-50 md:hidden shadow-[0_-5px_20px_rgba(0,0,0,0.5)] animate-slide-up">
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            className="flex-1 bg-gray-800 hover:bg-gray-700 text-white py-3 rounded font-bold border border-gray-600"
                        >
                            - 레벨 다운
                        </button>
                        <button
