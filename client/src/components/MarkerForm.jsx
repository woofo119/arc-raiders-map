...
const MarkerForm = ({ position, onClose }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        // 관리자 코드 확인 (임시: 'arcadmin')
        const isOfficial = isAdminMode && adminCode === 'arcadmin';
                ...
                <div className="pt-2 border-t border-gray-800">
                    <label className="flex items-center gap-2 text-xs text-gray-500 cursor-pointer hover:text-gray-300">
                        <input
                            ...
                            className="rounded border-gray-700 bg-black/50 text-arc-accent focus:ring-0"
                        />
                        <Shield size={12} />
                        관리자 모드 (공식 마커)
                    </label>
                    {isAdminMode && (
                        <input
                            ...
                            className="mt-2 w-full bg-black/50 border border-red-900/50 rounded p-1 text-xs text-red-200 placeholder-red-900/50 focus:outline-none"
                        />
                    )}
                </div>
