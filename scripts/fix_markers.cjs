...
async function main() {
    try {
        ...
        // Offset values calculated from user feedback
        // Raw (328, 590) -> Target (729, 658)
        // Delta X = 401
        // Delta Y = 68
        const OFFSET_X = 401;
        const OFFSET_Y = 68;
        ...
        while ((match = markerRegex.exec(content)) !== null) {
            ...
            if (mapping) {
                // New Transformation Logic: Translation + Y-Flip (Corrected Reference)
                // Reference: Extraction at Raw(553, 736) -> Target(729, 658)
                // (Chosen because it's closest to Apricots, matching Image 1)
                // Formula:
                // X = RawX + 176
                // Y = 1394 - RawY

                let targetX = rawX + 176;
                let targetY = 1394 - rawY;
